...
- ESP firmware scope: [esp/REQUIREMENTS_ESP.md](esp/REQUIREMENTS_ESP.md)
...
2. ESP firmware second because ESP8266 and ESP32-S2 hardware behavior depends on API command semantics.
...
### Phase 2: ESP Firmware Integration Against API

- Complete the firmware checklist in [esp/REQUIREMENTS_ESP.md](esp/REQUIREMENTS_ESP.md).
...
- ESP8266 and ESP32-S2 PlatformIO environments build successfully where supported hardware is available.
- ESP32-S2 Root CA generation and expiry handling are verified.
...
- API request and response schema is consumed correctly by Guest UI and supported ESP firmware targets.
...
- ESP8266 certificate fingerprint validation and ESP32-S2 Root CA validation are verified for HTTPS communication.
