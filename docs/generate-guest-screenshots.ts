...
async function generateAdminScreenshots(browser: Browser) {
...
async function fillAndSubmitGuestForm(page: Page, apartmentId: string, code: string): Promise<void> {
  await page.fill("#apartment", apartmentId);
  await page.fill("#code", code);
  await page.click("button.cta");
}

async function generateGuestFlowScreenshots(browser: Browser) {
...
async function generateQrScreenshot(browser: Browser) {
  ...
  try {
    ...
    await page.route("**/api/admin/apartments**", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify([
...
async function generateGuestLanguageScreenshots(browser: Browser) {
